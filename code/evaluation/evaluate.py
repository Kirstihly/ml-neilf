    ...
    if export_brdf:
        for obj_name, tex_coord, coord_shape, coord_mask in eval_dataset.tex_coords:
            ...
            for i, brdf_name in enumerate(["base_color", "roughness", "metallic"]):
                out_map = np.zeros(
                    [coord_shape[0] * coord_shape[1], valid_output[i].shape[-1]]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path", type=str, default="./confs/synthetic_neilf_brdf.json"
    )
    ...
    parser.add_argument(
        ...
        default="latest",
        type=str,
        help="the timestamp of the model to be evaluated.",
    )
    parser.add_argument(
        "--checkpoint",
        default="latest",
        type=str,
        help="the checkpoint of the model to be evaluated",
    )
